...
class Binomial(Distribution):
    ...
    def replace_stats_with_data(self):
        ...
        positive_trials_count = len(data_array[data_array == 1])
        self.p =  positive_trials_count / self.n
    ...
    def pdf(self, k):
        ...
        factorial_function = math.factorial(self.n) / (math.factorial(k) * math.factorial(self.n - k))

        return factorial_function * math.pow(self.p, k) * math.pow(1 - self.p, self.n - k)
    ...
    def plot_histogram_pdf(self):
        ...
        for specific_k in range(0, self.n + 1):

            pdf_result = self.pdf(specific_k)
            
            lists_of_k.append(specific_k)
            lists_of_pdf.append(pdf_result)

        plt.histogram(x = lists_of_k, y = lists_of_pdf)
        plt.title('Method for the PDF of the Binomial Distribution')
        plt.xlabel('Specific K')
        plt.ylabel('Probability')

        return lists_of_k, lists_of_pdf
    ...
    def __add__(self, other):
        ...
        if self.p == other.p:
            ...
            raise Exception('P values of the distributions must be equal') 
    ...
    def __repr__(self):
        ...
        return 'Mean {0}, Standar Deviation {1}, P {2}, N {3}'.format(self.mean, self.stdev, self.p, self.n)
